#!/usr/bin/env node
/**
 * Decide whether to patch-bump and npm publish signalk-telegram-notifications.
 *
 * Publishes at most once per UTC day when index.js changed since the last
 * npm release. If package.json is already ahead of npm, publish that version
 * as-is. First publish (package missing on npm) uses package.json as-is.
 *
 * Usage:
...
const PACKAGE_NAME = 'signalk-telegram-notifications'
const TRACKED_PATHS = ['index.js']
...
function nextPublishVersion(localVersion, npmVersion) {
  if (!npmVersion) return localVersion
  if (compareVersions(localVersion, npmVersion) > 0) return localVersion
  return bumpPatch(npmVersion)
}
...
function shouldPublish(opts) {
  ...
  if (files.length === 0) {
    return {
      publish: false,
      reason: 'no index.js updates since last publish',
      files: []
    }
  }
  return {
    publish: true,
    reason: 'index.js updated since last publish',
    files: files
  }
}
  ...
  try {
    ...
    if (err && err.code === 'NPM_MISSING') {
      return {
        publish: false,
        reason:
          'package not on registry; first publish must be from a logged-in machine',
        files: [],
